...
def run_anonymisation(text):
    ...
    for m in re.finditer(r'\b\d{4}[-\s]\d{4}[-\s]\d{4}\b', processed):
        ...
        processed=processed.replace(m.group(),t,1)
    # Phone +91 XX XXXX XXXX or 10-digit
    ...
    for m in re.finditer(r'\b[6-9]\d{9}\b', processed):
        t=tok("PHONE"); rec(t,m.group(),"Phone Number")
        processed=processed.replace(m.group(),t,1)
    # Structured IDs like LH-MUM-042, PT-001, SITE-DEL-001
    for m in re.finditer(r'\b(PT|SITE|IND|CT|SUBJ|INV|LH|MH|DL|CH)[-]\w{2,8}[-]\w{2,8}\b', processed):
        o=m.group()
    ...
    # Dates
    for pat in [
        re.compile(r'\b\d{1,2}[-/]\w{2,9}[-/]\d{2,4}\b'),
  ...
  <div class="hero-row">
    <div>
      ...
      <div class="hero-badges">
        ...
        <span class="hbadge g">✓ CDSCO Schedule Y</span>
        <span class="hbadge">Word · PDF · Audio</span>
        <span class="hbadge">6 AI Features</span>
      </div>
    </div>
    <div class="stat-row">
      <div class="sbadge"><span class="n">6</span><span class="l">Features</span></div>
      <div class="sbadge"><span class="n">20</span><span class="l">Sched. Y</span></div>
...
<div class="how-to">
  ...
  <div class="hw-steps">
    <div class="hw-s"><span class="hw-n">1</span>Choose a feature tab</div>
    <div class="hw-s"><span class="hw-n">2</span>Upload Word/PDF <b>or</b> paste text</div>
    <div class="hw-s"><span class="hw-n">3</span>Click the action button</div>
    <div class="hw-s"><span class="hw-n">4</span>Review AI-detected items</div>
    <div class="hw-s"><span class="hw-n">5</span>Download your report</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with t1:
    ...
    # ── Upload zone ───────────────────────────────────────────────────────────
    st.markdown('<div class="upload-zone"><h4>📁 Upload your document</h4>', unsafe_allow_html=True)
...
with t2:
    ...
    st.markdown('<div class="upload-zone"><h4>📁 Upload document</h4>', unsafe_allow_html=True)
...
with t3:
    ...
    with col_a:
        st.markdown('<div class="upload-zone"><h4>📁 Upload application document</h4>',unsafe_allow_html=True)
